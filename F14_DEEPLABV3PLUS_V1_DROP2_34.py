...
class Atrous_Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, rate=1, downsample=None):
        ...
        self.conv1 = nn.Conv2d(inplanes, planes, kernel_size=3, padding=rate, bias=False)
...
class Atrous_ResNet_features(nn.Module):
    ...
    def __init__(self, block, layers, pretrained=False):
        ...
        self.layer2 = self._make_layer(block, 64, layers[1], stride=1, rate=1)
        self.layer3 = self._make_layer(block, 128, layers[2], stride=1, rate=1)
        self.layer4 = self._make_MG_unit(block, 256, stride=1, rate=2)
    ...
    def _make_MG_unit(self, block, planes, blocks=[1,2,4], stride=1, rate=1):
        ...
        if stride != 1 or self.inplanes != planes * block.expansion:
            downsample = nn.Sequential(
                nn.Conv2d(self.inplanes, planes * block.expansion,
                          kernel_size=1, stride=stride, bias=False),
                nn.BatchNorm2d(planes * block.expansion),nn.ReLU(),nn.Dropout(0.5)
            )
        """
        layers = []
